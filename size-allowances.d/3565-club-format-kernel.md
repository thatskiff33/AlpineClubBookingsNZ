...
sites in one change. Seventy-six files that were already over their budgets grow
...
reason: 2 lines: `useClubFormat()` at the top of each component that renders an amount, and
  the club's format on every `formatCents` call beneath it. A hook has to sit in
  the component that renders, so there is no seam to lift it through; the file
  was several times its ceiling before this change added the argument.

file: src/app/(admin)/admin/fees/_components/hut-fees-section.tsx
lines: 1275
reason: 1 line: a `format: ClubFormat` parameter on the functions that render money, threaded
...
lines: 2662
reason: 1 line: `useClubFormat()` in each component that renders money and the argument on
  each formatter call, including the module-level helpers that build amount
  strings, which now take `format` rather than reaching for a hook they cannot
  call. The component was already far over budget; splitting it is its own change.

file: src/components/admin/manual-refund-task-queue.tsx
lines: 2028
reason: 7 lines: `useClubFormat()` in each component that renders money and the argument on
  each formatter call, including the module-level helpers that build amount
  strings, which now take `format` rather than reaching for a hook they cannot
  call. The component was already far over budget; splitting it is its own change.
...
lines: 1286
reason: 17 lines: a `format: ClubFormat` parameter on the functions that render money, threaded
  to each formatter, template and Xero call, so this module never reads the setting
  for itself — never inside a transaction and never per amount. The argument is
  the mechanism the compiler checks; hiding it in a helper would defeat that, and
  the file's debt predates this change by hundreds of lines.
...
lines: 1027
reason: 20 lines: a `format: ClubFormat` parameter on the functions that render money, threaded
  to each formatter, template and Xero call, so this module never reads the setting
  for itself — never inside a transaction and never per amount. The argument is
  the mechanism the compiler checks; hiding it in a helper would defeat that, and
  the file's debt predates this change by hundreds of lines.
...
lines: 3184
reason: 25 lines: a `format: ClubFormat` parameter on the functions that render money, threaded
  to each formatter, template and Xero call, so this module never reads the setting
  for itself — never inside a transaction and never per amount. The argument is
  the mechanism the compiler checks; hiding it in a helper would defeat that, and
  the file's debt predates this change by hundreds of lines.
...
lines: 1794
reason: 28 lines: a `format: ClubFormat` parameter on the functions that render money, threaded
  to each formatter, template and Xero call, so this module never reads the setting
  for itself — never inside a transaction and never per amount. The argument is
  the mechanism the compiler checks; hiding it in a helper would defeat that, and
  the file's debt predates this change by hundreds of lines.
