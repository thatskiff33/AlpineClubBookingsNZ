...
One file needs an allowance of its own. Four others this change also grows —
`src/app/(admin)/admin/book/page.tsx`, `src/lib/booking-create.ts`,
`src/lib/xero-booking-invoices.ts` and `src/lib/xero-sync.ts` — already carry a
...
is not the file's real length is exactly the drift it replaced. Those four
