...
/**
 * #2872 (CT-3, epic #2988) — twelve calendar-date columns narrowed from
 * `timestamp(3)` to `date`.
 *
...
const NARROWED_COLUMNS: { table: string; column: string; type: string }[] = [
  ...
  {
    table: "MembershipNominationSettings",
    column: "gateEffectiveFrom",
    type: "date",
  },
...
const narrowCalendarDateColumns: DataMigrationVerification = {
  ...
  intent:
    "Twelve columns that hold a CALENDAR DAY — three dates of birth, a membership start date, a life-membership date, two family-request dates of birth, four promo-window edges, a nomination-gate start day and a group-booking join deadline — become PostgreSQL `date` instead of `timestamp(3)`. Every stored day must survive the cast unchanged, every NULL must stay NULL, no neighbouring column may move, and a stored value that carries a TIME must stop the migration rather than be silently truncated onto a day nobody chose.",
  cases: [
    {
      name: "a club holding a value in all twelve columns, every one already a clean calendar day",
      seed: `
        INSERT INTO "Member" (
        ...
        VALUES ('default', true, TIMESTAMP '2026-06-15 00:00:00', TIMESTAMP '2026-08-25 09:41:07.123');
      ...
      expectations: [
        {
          claim:
            "all twelve columns are PostgreSQL `date` afterwards. This is the claim that fails when the migration is not applied at all — every day assertion below would pass over an untouched `timestamp` column, because to_char reads the same either way",
        ...
        {
          claim:
            "the nomination gate's start day and the group booking's join deadline survive",
          sql: `SELECT (SELECT to_char("gateEffectiveFrom", 'YYYY-MM-DD')
                          FROM "MembershipNominationSettings" WHERE "id" = 'default') AS "gateEffectiveFrom",
                       (SELECT to_char("joinDeadline", 'YYYY-MM-DD')
                          FROM "GroupBooking" WHERE "id" = 'cd-group') AS "joinDeadline"`,
          rows: [
            { gateEffectiveFrom: "2026-06-15", joinDeadline: "2026-08-30" },
          ],
        },
        {
          claim:
            "the booking's own lodge nights are untouched. `Booking.checkIn`/`checkOut` were ALREADY `@db.Date` and are the model this migration copies, so a change here would mean the migration had reached past its twelve columns",
    ...
    },
    {
      ...
      expectations: [
        {
          claim:
            "the twelve columns narrow with no rows present at all. This is the case that would pass vacuously on its own, which is exactly why it asserts the TYPE rather than a value",
          sql: NARROWED_COLUMN_TYPES_SQL,
          rows: NARROWED_COLUMNS,
        },
      ],
  ...
  mutants: [
    {
      name: "invert the preflight's own predicate on Member.dateOfBirth, so it treats a clean midnight value as an offender",
      harm:
        "Not a shipping hazard — it is the proof that the fail-closed preflight is REACHABLE and really evaluates the rows a case seeds. Every other assertion in this fixture is about the cast, and all of them would pass with the whole DO block deleted. Inverting the comparison makes the seeded clean rows match, so the migration must RAISE; if it does not, the block is not running against these tables and the one guard standing between a club's dates of birth and a silent truncation is decoration.",
      find: `AND "dateOfBirth" <> date_trunc('day', "dateOfBirth")`,
      replace: `AND "dateOfBirth" = date_trunc('day', "dateOfBirth")`,
    },
    {
    ...
    },
    {
      ...
      harm:
        "An off-by-one on a deadline closes group joining a day early, so the last member to try is refused a booking the organiser had left open for them. It is here as well as the date-of-birth mutant because the two columns are on different tables and only one of the twelve statements is being tested by that one: a fixture whose teeth are all in a single ALTER would pass while eleven others did the wrong thing.",
    ...
    {
      name: "forget one of the twelve columns — Member.lifeMemberDate is never narrowed",
      harm:
