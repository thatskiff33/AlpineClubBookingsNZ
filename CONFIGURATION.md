...
| Variable                           | Description                                                                                          |
| ---------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `CURRENCY`, `NEXT_PUBLIC_CURRENCY` | **A seed only, since #3563.** The club's currency is now recorded in the database and edited in-app at **Admin → Setup & Configuration → Club Currency & Locale** (`/admin/club-format`); see [`docs/guides/club-format.md`](docs/guides/club-format.md). These variables are read for exactly one purpose: on the first start after upgrading, an installation that has no recorded currency copies the value it is *already effectively using* from here, so nothing about its behaviour changes. **After that the recorded setting is the authority for the SETTING, and editing these variables no longer changes it.** **Since #3565 (money) and #3566 (dates) nothing on screen or in an email reads them**: every amount, every date and time — emails included, which re-read the recorded setting as soon as it is saved — the currency AI spend is counted in, and the site's alphabetical sorting all follow the recorded setting, so an operator who edits or unsets `CURRENCY` / `LOCALE` changes nothing a club sees. Keep them only as the first-start seed. The one surface still derived from `CURRENCY` is the card-payment currency sent to the payment provider (`APP_STRIPE_CURRENCY`), which #3567 decides. `NZD` is the generic New Zealand default and applies only where neither a recorded currency nor these variables say anything. Store a three-letter ISO 4217 code (`NZD`, `CHF`); a symbol or a name is refused. The `APP_CURRENCY` / `APP_LOCALE` / `APP_STRIPE_CURRENCY` constants in `src/config/operational.ts` still derive from these; after #3566 no module outside that file reads the first two (the seed reader aside), and #3567 retires them. Changing the setting re-denominates nothing: every stored amount stays the integer cents it was. |
