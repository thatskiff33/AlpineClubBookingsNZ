...
| Variable                           | Description                                                                                          |
| ---------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `CURRENCY`, `NEXT_PUBLIC_CURRENCY` | **A seed only, since #3563.** The club's currency is now recorded in the database and edited in-app at **Admin → Setup & Configuration → Club Currency & Locale** (`/admin/club-format`); see [`docs/guides/club-format.md`](docs/guides/club-format.md). These variables are read for exactly one purpose: on the first start after upgrading, an installation that has no recorded currency copies the value it is *already effectively using* from here, so nothing about its behaviour changes. **After that the recorded setting is the authority and editing these variables changes nothing** — an operator who edits `CURRENCY` on the server and expects a screen to follow will be disappointed, which is why the admin screen says so too. `NZD` is the generic New Zealand default and applies only where neither a recorded currency nor these variables say anything. Store a three-letter ISO 4217 code (`NZD`, `CHF`); a symbol or a name is refused. The transitional `APP_CURRENCY` / `APP_STRIPE_CURRENCY` constants still derive from these for the call sites programme #3205 has not migrated yet (#3564-#3566), and #3567 retires them. Changing the setting re-denominates nothing: every stored amount stays the integer cents it was. |
