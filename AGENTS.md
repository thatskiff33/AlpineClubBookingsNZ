...
| What time it is **at the club** — the installation's timezone, or anything reading `TZ` / `NEXT_PUBLIC_TZ` / `APP_TIME_ZONE` | `INV-CONFIG-002` → [`product-configuration.md`](docs/invariants/product-configuration.md) | [`guides/club-time.md`](docs/guides/club-time.md); format and derive through [`club-time`](docs/CLUB_TIME_KERNEL.md), never by hand |
