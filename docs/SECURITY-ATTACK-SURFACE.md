...
| `/api/availability`, `/api/availability/check`, `/api/booking-policies/check`, `/api/promo-codes/available`, `/api/promo-codes/validate` | Authenticated active member. | Signed-in member. | Capacity, booking policies, promo definitions, age-tier data. | None directly. | Active-session checks; availability and promo validation use `rateLimiters.bookingQuery`. Zod validation on policy and promo inputs. On the multi-lodge availability/pricing reads (`/api/availability`, `/api/availability/check`, `/api/bookings/quote`, `/api/bookings/rooms`) the resolved lodge is passed through `isMemberEligibleToBookLodge` (mirroring the booking create path) so a `BOOKING_RESTRICTION`ed member gets `403` instead of reading a forbidden lodge's availability/pricing; admin on-behalf quotes bypass it as the audited override. Since #2664 (create side) `/api/bookings/rooms` has no production caller for its no-`lodgeId` cross-lodge listing mode: the booking wizard was the last remaining client (the requested-room editor was the other, closed by the same issue's read-side fix — see the `/api/bookings/[id]/**` row below) and it now always names its lodge, so nothing reaches that mode during the pre-selection window on every mount, nor stickily whenever a reply landed after the one that superseded it. Worth recording precisely, because it was previously described too kindly: the mode's filter was `Room.active` plus the member's `BOOKING_RESTRICTION` set and **not** the lodge's own `active` flag, so an unrestricted member's listing returned archived lodges' room names too. #2727 closed that — the unscoped listing now also filters on `Lodge.active`, for a default-open member and for a restricted member whose eligible set contains a since-archived lodge, so a discovery read never offers a property the club has taken out of service. The named-lodge mode is unchanged (still `403` for a barred member, still answering for an explicitly named archived lodge), and the unscoped mode is still served rather than retired. Since #2678 this is a RULE rather than a description: `INV-INT-016` (`docs/invariants/integrations.md`) records that the mode is retained for FORKED and EXTERNAL consumers — it is the pre-multi-lodge signature and their booking wizards still call it that way — that no client in `src/` may use it, because internal reuse of the unscoped mode is what produced the #2664 wrong-lodge pickers, and (since #2727) that the archived-lodge exclusion is part of what retaining it means. A guard test sweeps `src/` for a call omitting `lodgeId`, and `src/app/api/bookings/rooms/__tests__/rooms-route-lodge-scope.test.ts` exercises both branches, since no internal caller does. | Logger on errors. | These are not anonymous in current code despite being booking-discovery shaped. #613 should keep member-read helpers distinct as this family grows. (Season CRUD now lives solely under `/api/admin/seasons`; the duplicate member-open `/api/seasons` route was removed in #1252.) |
...
| `/api/bookings/[id]/**`, including cancel, modify, guests, payment confirmation, refund request, waitlist confirmation, notes, arrival time, and payment secret routes | Authenticated active member with route/service ownership checks. | Booking owner/member, sometimes admin through service rules. | Booking ownership, guest records, payment transactions, Stripe IDs/client secrets, cancellation/refund/change-request data, notes. | Stripe PaymentIntent/SetupIntent confirmation or retrieval, Xero invoice/credit-note outbox, email notifications. | Auth.js session, active-account guard, rate limits on cancel/change flows, service-level owner checks, Zod/date validation. | Audit logs for payment/guest/refund/change operations where implemented; logger on failures. Since #2621 the expected-arrival-time `PUT`/`DELETE` also audit (`booking.expected_arrival_time.set` / `.cleared`, category `booking`, with `entityType`/`entityId`), naming the booking's OWNER as subject and the caller as actor, with an `onBehalf` flag for an officer acting under #1313 option A2. The old value is captured inside the same transaction as the write, so concurrent saves chain honestly rather than both claiming the same predecessor, and the clear carries the value it destroyed — which is the only surviving record of it. Both are written after the update commits, so a refused value, a stale booking or a permission refusal records nothing. Since #2664 the requested-room picker's option list is also booking-scoped (`GET /api/bookings/[id]/requested-room/options`): it resolves the booking, applies the same owner-or-Full-Admin-or-`bookings:edit` predicate, and lists only that booking's lodge's ACTIVE rooms, with the lodge taken from `Booking.lodgeId` server-side — no caller-supplied `lodgeId` is read, and member booking eligibility (`isMemberEligibleToBookLodge`) is deliberately not consulted, because the authority is the booking rather than the caller's ability to book that lodge. It replaced an unscoped `/api/bookings/rooms` read that both leaked other lodges' room inventory into the picker and filtered a Booking Officer's choices by their own personal booking restrictions. A soft-deleted booking answers `404` here to EVERY role — no Full Admin exemption, unlike the booking page, because the picker this feeds is hard-disabled on a deleted booking for every role — and that check sits after the authorisation check so an unauthorised caller gets `403` either way rather than a deleted-or-live oracle. Two exposures are disclosed rather than closed: a member restricted away from a lodge AFTER taking a booking there can still read that booking's room names, which is deliberate because `writeRequestedRoom` never consults `assertMemberMayBookLodge` and so that member may still change the room (refusing the read while permitting the write is the defect this route removed); and the `404`-before-`403` ordering confirms a booking id exists to an authenticated caller, which is byte-identical to every neighbour in `/api/bookings/[id]/**` and only ever confirms a cuid the caller already holds. Since #2674 that same shape is the house rule for this row, written down as `INV-ADDPAY-031` (`docs/invariants/additional-payment-chasing.md`) rather than restated per route, and three more methods now follow it: `arrival-time` `PUT`/`DELETE` and `refund-request` `POST` each answer `404` to EVERY role including Full Admin, placed AFTER the authorisation check. `send-guest-payment-link` `POST` already consulted `deletedAt` but consulted it FIRST, so an unauthorised caller got `403` on a live booking and `404` on a deleted one — a deleted-or-live oracle on any id whose existence they could otherwise establish; it was reordered by the same issue, which is why "this route checks `deletedAt`" is not on its own evidence that it follows the rule. `exception-requests/[requestId]` `PATCH` has no authorisation step in front of its claim, so it refuses through a `booking: { deletedAt: null }` filter ON the guarded `updateMany` instead: a deleted booking loses the claim and produces the same bare `409`, no audit and no reservation release as a wrong-owner or wrong-status loss, so it is indistinguishable from either. A verified sweep of all 27 exported methods on this prefix is recorded under `INV-ADDPAY-030`, re-measured by #2700 to **11 refusing on `deletedAt` directly / 15 refusing incidentally / 1 deliberately writing anyway / 0 unguarded reads**. #2700 closed the last three of those, settling what `INV-ADDPAY-032` and `INV-ADDPAY-033` had tracked as open decisions (both are now superseded stubs). Three consequences matter for this row. First, `guests/[guestId]/consent` `POST` (`INV-ADDPAY-035`) now refuses a soft-deleted booking for every role on BOTH arms, recording nothing and emailing nobody; the guard is asserted twice — once unlocked, once inside the transaction under `pg_advisory_xact_lock(1)` — because `softDeleteCancelledBooking` takes the same key, so a deletion committing between the two reads is still seen. Second, that refusal and the two reads `change-requests` `GET` and `refund-request` `GET` answer a **deliberately informative** body — "cancelled or removed" — rather than `INV-ADDPAY-031`'s byte-identical not-found body (`INV-ADDPAY-034`). That is a named carve-out from the BODY half of the house rule and from that half only: the ORDERING half has no exception anywhere, every one of the three sits after its authorisation check, and a caller with no claim still receives `403` whose answer does not move with the booking's deletion state — so nothing is disclosed to anyone not already entitled to the record, and the message names neither the actor who deleted it nor the booking owner. Note `refund-request` therefore carries BOTH bodies: its `POST` keeps `Booking not found` from #2674 and its `GET` carries the shared sentence, which is asserted rather than accidental. Both halves of the carve-out are enforced by a source sweep rather than left to review: `src/lib/__tests__/deleted-booking-refusal-callers.test.ts` fails on a FOURTH importer of the shared sentence and asserts per surface that the guard sits below that surface's own authorisation refusal — the specific regression being that an informative body is now normalised here, so importing the constant is easy and inheriting the ordering decision by import is the easy mistake. Worth stating plainly for anyone weighing the carve-out's cost: today no FRESH navigation reaches any of the three bodies. The booking page `notFound()`s a non-admin on a deleted booking, the delegate consent page resolves `NOT_FOUND` above both its target and delegate branches, the refund-appeal button is not rendered on a deleted booking, and `change-requests` `GET` has no client in `src/` or `e2e/` at all. The reader is a client that loaded the booking before the deletion and acted after it — the same race the rest of this row covers — or a direct API caller. Widening it to the email journey the decision described would mean changing the delegate page's uniform `NOT_FOUND`, itself a privacy property with its own Playwright assertion, and is an owner decision rather than a tidy-up. Third, `confirm-modification-payment` `POST` (`INV-ADDPAY-036`) is the one method here that consults `deletedAt` and deliberately writes anyway: Stripe has already captured, so refusing would leave captured money with no ledger row, and instead the payment is recorded and an OPEN `ManualRefundTask` is raised for a human — never an automatic refund from that path. The booking-deletion path now also cancels the booking's in-flight PaymentIntents after its transaction commits (provider call outside the lock, never throwing, ledger marked FAILED only when Stripe confirms the cancel), which makes that race rare rather than impossible. A refund can still occur from the older #1350 webhook path, which auto-refunds any additional capture on a `CANCELLED` booking and therefore covers deleted ones; when it does, it closes the raised task as `DISMISSED` so an operator cannot complete it and write a second refund allocation for one refund. | IDOR and money-state risk. #614 should include representative owner-boundary tests; #617 should review transaction invariants. |
