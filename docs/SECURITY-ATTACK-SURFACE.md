...
- That job no longer runs the bare `npm audit --audit-level=high`. The command
  ...
  mistaken for a finding or the reverse (#3254). Green still means the audit ran;
  the accepted cost is that a sustained npm outage blocks merges. A vulnerability
  is never retried, and there is exactly one code path that can exit 0 - the one
  holding parsed severity counts - so an unparseable or missing report fails
  closed.
