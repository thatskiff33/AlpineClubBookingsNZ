...
- **A `"use client"` module never reaches server-only code (#2686).**
  ...
  **Which modules carry the marker, and how the operator CLIs live with it.**
  ...
  benefit had already killed the `E2E multi-lodge` gate exactly that way
  (#3056).
...
- **Reclassifying an audit row's `category` in code changes only the rows written
  ...
  **Enforcement is partial, and the boundary is stated rather than implied.** The
  ...
  than having none: the audit-writer census pins only 127 of its 468 write sites
  ...
  something edit a 400-line literal — so for the other 337 there is no baseline
