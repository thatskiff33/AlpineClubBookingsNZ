...
| --- | --- | --- | --- |
| **Calendar date** | `CalendarDate` — a branded `YYYY-MM-DD` string | **never** | a lodge night, a birthday, a season start, a promo window |
...
**Noon needs no policy at all.** In the same sweep, local noon is never skipped
and never ambiguous in any zone — which is the measured reason the epic's
noon-to-noon stay boundary is safe where a midnight boundary would not be.
