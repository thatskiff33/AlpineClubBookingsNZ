...
- **A shared `src/lib` module a CLI can reach — and only then.** Use
  ...
  **"Usually", because a green census is not a licence to move a SCHEDULED JOB.**
  ...
  `payment-link-reissue.ts` are reached only from the `/pay` routes. The
  context, re-issue and split-guest modules keep the outside-request reader
  (`payment-link-intent.ts` derives no civil date and reads no zone): a split
  moves code and does not change which client reads the club's zone, because
  that is an `INV-CONFIG-002` decision and would be its own change, not a
  refactor. Where such a module also runs under a lock, it resolves the zone
  before the transaction and threads it in as a value; see
