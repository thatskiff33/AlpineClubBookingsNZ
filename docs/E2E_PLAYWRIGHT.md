...
| `e2e/raw-css-secret-reflection.spec.ts` | No matrix row - regression guard for #2981 (Critical) | The one thing no source scan and no jsdom test can answer: whether a typed secret becomes visible to a CSS selector in a real browser. The spec saves real administrator Raw CSS through `PUT /api/admin/site-style` containing a control rule plus the full `[value^="..."]` prefix ladder for a known kiosk PIN, opens the anonymous hut-leader instructions page (`?a=` only has to be PRESENT for the PIN form to render, so no assignment is seeded and seed drift cannot break it), and after every keystroke - and after paste, a mid-string edit, the rerender a refused submit causes, and a clear - asserts all four layers at once: the `value` property holds the typed prefix, `getAttribute("value")` is `null`, not one of the eight oracle selectors matches, and the computed `outline-color` is still the control rule's. The control rule is what stops the whole spec passing vacuously on a page the Raw CSS never reached. Restores the club's original `rawCss` in `afterAll`. The static half - which fields are owed a `SecretInput`, and that `SecretInput` still passes no value into the DOM - is `src/lib/__tests__/raw-css-secret-input-census.test.ts` |
