...
This directory is the durable source of truth for integrating the separate `tac-dashboard` finance dashboard into `TACBookings`.

Use these documents for all future sessions. Chat history is not the source of truth.
...
3. GitHub PRs carry implementation, test evidence, and next-step handoff.
