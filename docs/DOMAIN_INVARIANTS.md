...
| `INV-MONEY-028` | Every stored guest-night price records its origin; an even split is not sold-price evidence |
| `INV-MONEY-029` | Promo adjustment rows per night or guest reconcile to the recorded promo totals; unknown is never zero |
...
| `INV-PAY-047` | The generalised ledger mirror, net of refunds and carrying the change fee: every cent is collected, paid with credit, or still owed as an ask sized to keep it true |
...
| `INV-PAY-054` | An unusable saved card is terminal for the cron: retired everywhere, escalated once |
| `INV-PAY-055` | A saved-card charge attempt is one durable ledger row with its own Stripe key; an unresolved attempt is replayed, a definite failure ends it, and the never-double-charge guard lives on the ledger under the claim's locks |
| `INV-PAY-056` | A payment recovery becomes terminally failed in exactly one place, and that place owns what a dead recovery costs |
| `INV-PAY-057` | A dead additional-payment recovery withdraws the ask it left standing, so nothing can be paid against an invoice raised unpaid |
