...
| `INV-MOD-025` | An in-progress edit prices the nights a guest holds, not their envelope, sells only the nights it creates, and values each at the price it was sold for — or, where that cannot be recovered, from the member's own stored booking price — never leaving a guest owing less than nothing; ordinary stays are unchanged and history is not repriced |
