...
| `INV-PAY-052` | A payment recovery becomes terminally failed in exactly one place, and that place owns what a dead recovery costs |
| `INV-PAY-053` | A dead additional-payment recovery withdraws the ask it left standing, so nothing can be paid against an invoice raised unpaid |
