...
this release except that two migrations — `20260928020000` and `20260928030000` —
...
docker compose exec -T postgres \
  sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
  < prisma/migrations/20260928030000_backfill_school_bookings_to_organisations/rollback.sql

docker compose exec -T postgres \
  sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' \
  < prisma/migrations/20260928020000_booking_owner_optional_member/rollback.sql
```
...
| `school_backfill_rollback_xero_unreconstructable` from `20260928030000/rollback.sql` | A school record holds a Xero customer whose original owner cannot be proved from what is left. Returning it to the wrong member would misattribute a provider identity permanently and invisibly. | Stop, and restore from the backup as above. Do not hand the customer back by hand without checking Xero's own history first. |
