...
| Nobody noticed a booking's invoice had failed until the member chased it | Until #3001 the only evidence lived here, under **Operations**, and the booking itself looked normal | It now says so on the booking. A booking whose invoice operation is **Failed** or part-finished carries an admin-only warning on its own page naming what Xero does and does not have, with a link back to this screen. **Resolve (fixed in Xero)** on the operation clears that warning too, without touching the booking |
