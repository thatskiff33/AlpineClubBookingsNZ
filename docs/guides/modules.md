...
| Alpine Central Server (`alpineCentralServer`) | Connects this club to the Alpine Central Server (ServerNZ) to upload and download data shared across clubs, starting with the Other Clubs registry. **Shares data outside your club**: your lodges' names, locations, bed counts and booking-officer contact details go to the central server and on to every other connected club. Nothing is sent until you also enable an item on its setup page. Needs an API key from the central server, entered under **Admin › Integrations › Alpine Central Server**. See [Integrations](integrations.md) | Off |
...
| Member lodge roster (`memberLodgeRoster`) | Lets a signed-in member see who else is staying, for the next 30 nights, at a lodge they can already book. Names and nights only: no contact details, no prices, no booking information. **Shows your members to each other** — a member's name and the nights they are staying become visible to every other member with access to that lodge, and an individual cannot hide themselves. How much of a name is shown is set per lodge under **Admin › Lodges › the lodge › Member roster**, and defaults to the full name. A booking that includes a child never names anyone in it: it shows a family or group label instead, at every setting. Off by default because switching it on is a decision about showing your members to one another | Off |
