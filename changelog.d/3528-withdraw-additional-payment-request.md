- **A finance officer can now withdraw an unpaid payment request that a
  ...
  the amount off the booking, retires any Xero invoice that was waiting on that
  payment without sending it, and stops any background retry from raising the
  request again. The member no longer sees anything owing. The review that
  ...
  Only a request raised by a financial review can be withdrawn. A request that
  comes from a change to the booking's price is the price itself — if that is
  wrong, edit the booking, which replaces the request. A request the member
