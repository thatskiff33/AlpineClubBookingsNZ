- **Maintainability: the member/admin booking-detail page is decomposed by
  domain responsibility with no change in behaviour.** The route page now keeps
  only the shell — sign-in, the club clock, the booking read,
  ...
  exactly as before. The module map and the measured before-and-after line
  counts are in `docs/MAINTENANCE.md` -> "Refactor history and split guidance".
  (#2958)
