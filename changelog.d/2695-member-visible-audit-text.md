- **The club now decides what a member reads on their own activity history,
  ...
  **Officers lose nothing:** every entry keeps its full wording on Admin → Audit
  ...
  Two smaller repairs ride along. An entry's short title is now held to the same
  secret and card-number rules as its note, which it previously skipped. And the
  title a member sees for a setup-invite or password-reset entry is no longer
  built out of the stored payload, which on an officer's own timeline could name
  a different member's email address.
