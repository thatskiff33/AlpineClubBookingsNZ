...
const mockEnqueueXeroBookingInvoiceUpdateOperation = vi
  .fn()
  .mockResolvedValue({
    queueOperationId: "op_booking_update",
    message: "queued"
  });
const mockEnqueueXeroSupplementaryInvoiceOperation = vi
  .fn()
  .mockResolvedValue({
    queueOperationId: "op_supplementary",
    message: "queued"
  });
const mockEnqueueXeroModificationCreditNoteOperation = vi
  .fn()
  .mockResolvedValue({
    queueOperationId: "op_mod_credit_note",
    message: "queued"
  });
const mockKickQueuedXeroOutboxOperationsIfConnected = vi
  .fn()
  .mockResolvedValue(null);
const mockRecordSkippedXeroBookingInvoiceUpdateOperation = vi
  .fn()
  .mockResolvedValue({ queueOperationId: "op_skip", message: "skipped" });
...
vi.mock("@/lib/promo", () => ({
  ...
  calculatePromoDiscountForGuestRates: vi
    .fn()
    .mockReturnValue({
      discountCents: 0,
      priceAdjustmentCents: 0,
...
vi.mock("@/lib/xero-operation-outbox", () => ({
  enqueueXeroBookingInvoiceUpdateOperation:
    mockEnqueueXeroBookingInvoiceUpdateOperation,
  enqueueXeroSupplementaryInvoiceOperation:
    mockEnqueueXeroSupplementaryInvoiceOperation,
  ...
  recordSkippedXeroBookingInvoiceUpdateOperation:
    mockRecordSkippedXeroBookingInvoiceUpdateOperation
}));
vi.mock("@/lib/logger", () => ({
...
const mockedCheckCapacityForGuestRanges = vi.mocked(
...
const mockedValidateAndCalculatePromo = vi.mocked(
  validateAndCalculatePromoDiscount
);
...
describe("PUT /api/bookings/[id]/modify-dates", () => {
  ...
  it("returns 401 for unauthenticated requests", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/bookings/bk1/modify-dates",
      {
        method: "PUT",
  ...
  it("cleans up out-of-range SUGGESTED chore assignments", async () => {
    ...
    expect(tx.choreAssignment.delete).toHaveBeenCalledWith({
      where: { id: "ca1" }
    });
...
describe("DELETE /api/bookings/[id]/guests/[guestId]", () => {
  ...
  it("returns 401 for unauthenticated requests", async () => {
    mockedAuth.mockResolvedValue(null as any);
    const req = new NextRequest("http://localhost/api/bookings/bk1/guests/g1", {
      method: "DELETE"
    });
  ...
  it("returns 404 if guest not found on booking", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/bookings/bk1/guests/nonexistent",
      { method: "DELETE" }
    );
    const res = await DELETE(req, {
      params: Promise.resolve({ id: "bk1", guestId: "nonexistent" })
    });
    expect(res.status).toBe(404);
    const body = await res.json();
  ...
  it("successfully removes guest and recalculates price", async () => {
    ...
    const req = new NextRequest("http://localhost/api/bookings/bk1/guests/g2", {
      method: "DELETE"
    });
    const res = await DELETE(req, {
