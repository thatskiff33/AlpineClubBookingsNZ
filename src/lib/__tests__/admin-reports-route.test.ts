...
vi.mock("@/lib/capacity", () => ({
  ...
  getOccupiedBedsForNight: vi.fn(
    (date: Date, bookings: Array<{ guests?: unknown[] }>) =>
      bookings.reduce(
        (total, booking) => total + (booking.guests?.length ?? 0),
        0
      )
  ),
  LODGE_CAPACITY: 29
...
describe("admin reports route", () => {
  ...
  it("returns daily revenue data and current-season member stats", async () => {
    ...
    expect(
      mockPrisma.booking.findMany.mock.calls[0][0].where.deletedAt
    ).toBeNull();
    expect(
      mockPrisma.booking.findMany.mock.calls[1][0].where.deletedAt
    ).toBeNull();
