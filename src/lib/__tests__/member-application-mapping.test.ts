...
function makeTarget(
  overrides: Partial<MappingTargetRecord> = {},
): MappingTargetRecord {
  return {
    id: "member-x",
    email: "old@test.com",
...
const applicantMapDecisions = (
  memberId: string,
): NormalizedPersonDecision[] => [
...
describe("computeApprovalMappingOutcomes — applicant MAP", () => {
  it("diffs name/email, flags promotion, and blocks a foreign login holder", async () => {
    ...
    const emailDiff = applicant.fieldDiffs.find(
      (diff) => diff.field === "email",
    );
    expect(emailDiff).toMatchObject({
      current: "old@test.com",
      incoming: "jane@test.com",
      willChange: true,
    });
  ...
  it("blocks mapping to a nominator, an inactive/archived member, and an already-grouped member", async () => {
    ...
    expect(inactiveOutcome.persons[0].errors.join(" ")).toContain(
    ...
    const grouped = makeTarget({
      id: "m-grouped",
      familyGroupMemberships: [{ familyGroupId: "fg-9" }],
    });
    const groupedOutcome = await computeApprovalMappingOutcomes({
      ...
      decisions: [
        {
          ref: { kind: "applicant" },
          decision: { mode: "MAP", memberId: "m-grouped" },
        },
    ...
    expect(groupedOutcome.persons[0].errors.join(" ")).toContain(
      "already belongs to a family group",
    );
  });
  ...
  it("skips billing with a note when the target already has season coverage", async () => {
    ...
    expect(persons[0].notes.join(" ")).toContain(
      "existing season membership coverage",
    );
  });
});
...
describe("computeApprovalMappingOutcomes — family MAP", () => {
  ...
  it("blocks an ADMIN member mapped as a dependent", async () => {
    ...
    expect(persons[1].errors.join(" ")).toContain(
      "admin member cannot be mapped as a dependent",
    );
  });

  it("sets the parent link only for a non-login target with no parent; notes otherwise", async () => {
    const fresh = makeTarget({
      id: "m-fresh",
      canLogin: false,
      parentMemberId: null,
    });
    ...
    expect(loginableOutcome.persons[1].notes.join(" ")).toContain(
      "left untouched",
    );
  });
  ...
  it("blocks the same member mapped to two people (duplicate target)", async () => {
    ...
    const { blockingErrors } = await computeApprovalMappingOutcomes({
      ...
      decisions: [
        {
          ref: { kind: "applicant" },
          decision: { mode: "MAP", memberId: "dup" },
        },
        {
    ...
    expect(blockingErrors.join(" ")).toContain(
      "cannot be mapped to more than one person",
    );
  });
  ...
  it("verifies a matching payload and rejects row-level and outcome-only drift", async () => {
    ...
    expect(
      verifyApprovalMappingPreviewToken(
        {
          application,
          persons: rowDrift.persons,
          blockingErrors: rowDrift.blockingErrors,
        },
        token,
      ),
    ).toBe(false);
    ...
    expect(
      verifyApprovalMappingPreviewToken(
        {
          application,
          persons: outcomeDrift.persons,
          blockingErrors: outcomeDrift.blockingErrors,
        },
        token,
      ),
    ).toBe(false);
...
describe("privileged-email mapping gate (#1026 parity)", () => {
  ...
  it("does not gate a same-email mapping, an unprivileged target, or a non-login target", async () => {
    ...
    const sameEmail = await computeApprovalMappingOutcomes({
      ...
      decisions: applicantMapDecisions("member-x"),
      targetsById: new Map([
        ["member-x", privileged({ email: "jane@test.com" })],
      ]),
      loginHolderId: "member-x",
      seasonYear: 2026,
      actor: SCOPED_ADMIN,
...
describe("privileged promotion mapping gate (#1604 parity, canLogin-blind)", () => {
  ...
  const outcomesFor = (
    ...
    actor: { id: string; isFullAdmin: boolean },
  ) =>
    computeApprovalMappingOutcomes({
      application: makeApplication(),
      decisions: applicantMapDecisions("member-x"),
  ...
  it("blocks the FINANCE_ADMIN access-role and legacy financeAccessLevel (dormant Treasurer) variants", async () => {
    const financeRole = await outcomesFor(
      dormant({
        accessRoles: [{ role: "FINANCE_ADMIN", roleDefinitionId: null }],
      }),
      SCOPED_ADMIN,
    );
...
describe("buildApprovalMappingPreview", () => {
  ...
  it("ranks an exact email match ahead of a name-only match", async () => {
    ...
    const body = result.body as {
      preview: {
        persons: Array<{
          suggestions: Array<{ id: string; matchedOnEmail: boolean }>;
        }>;
        hasMappings: boolean;
      };
    };
    expect(body.preview.hasMappings).toBe(false);
    expect(body.preview.persons[0].suggestions[0]).toMatchObject({
      id: "email-hit",
      matchedOnEmail: true,
    });
  });
