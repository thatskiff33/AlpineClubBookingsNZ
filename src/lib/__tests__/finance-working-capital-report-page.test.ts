...
function balanceSheetSnapshot(input: {
  ...
  totalEquity: string;
  omitCurrentAssetsSection?: boolean;
  omitCurrentLiabilitiesSection?: boolean;
}) {
  return {
    id: input.id,
      ...
      rows: [
        {
          ...
          cells: [],
          rows: [
            ...(input.omitCurrentAssetsSection
              ? []
              : [
                  {
                    rowType: "Section",
        ...
        },
        {
          ...
          cells: [],
          rows: [
            ...(input.omitCurrentLiabilitiesSection
              ? []
              : [
                  {
                    rowType: "Section",
...
describe("finance working-capital report page model", () => {
  ...
  it("builds a working-capital view from the latest stored balance-sheet snapshots", async () => {
  ...
  it("warns and skips stored snapshots without current working-capital sections", async () => {
    mockListFinanceSnapshots.mockResolvedValue([
      balanceSheetSnapshot({
        id: "snapshot-april-30",
        asOfDate: "2026-04-30",
    ...
    expect(model.coverageSummary).toBe(
      "Showing 1 stored working-capital snapshot from 30 Apr 2026 backwards."
    );
    expect(model.filterWarnings).toContain(
      "1 stored balance-sheet snapshot did not expose current-asset and current-liability sections and was ignored."
    );
    expect(model.periodRows).toHaveLength(1);
  });

  it("shows current ratio as unavailable when current liabilities are zero", async () => {
    mockListFinanceSnapshots.mockResolvedValue([
      balanceSheetSnapshot({
        id: "snapshot-april-30",
        asOfDate: "2026-04-30",
