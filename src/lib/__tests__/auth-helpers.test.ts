...
describe("computeAgeTierWithSettings (TAC boundaries: CHILD 5-12, YOUTH 13-17, ADULT 18+)", () => {
  ...
  it("returns INFANT for age under 5", () => {
    ...
    expect(
      computeAgeTierWithSettings(new Date("2025-01-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("INFANT");
    // Born 2021-04-02: age 4 on April 1 2026 -> INFANT
    expect(
      computeAgeTierWithSettings(new Date("2021-04-02"), ref, AGE_TIER_DEFAULTS)
    ).toBe("INFANT");
  });
  ...
  it("returns CHILD for age 5-12", () => {
    ...
    expect(
      computeAgeTierWithSettings(new Date("2021-04-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("CHILD");
    // Born 2017-01-01: age 9 on April 1 2026 -> CHILD
    expect(
      computeAgeTierWithSettings(new Date("2017-01-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("CHILD");
    // Born 2016-04-01: age 10 on April 1 2026 -> CHILD (CHILD goes up to 12)
    expect(
      computeAgeTierWithSettings(new Date("2016-04-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("CHILD");
    // Born 2016-04-02: still 9 on April 1 2026 (turns 10 on April 2) -> CHILD
    expect(
      computeAgeTierWithSettings(new Date("2016-04-02"), ref, AGE_TIER_DEFAULTS)
    ).toBe("CHILD");
    // Born 2016-01-01: age 10 on April 1 2026 -> CHILD
    expect(
      computeAgeTierWithSettings(new Date("2016-01-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("CHILD");
    // Born 2014-08-28 (Malia example): age 11 on April 1 2026 -> CHILD (under 13)
    expect(
      computeAgeTierWithSettings(new Date("2014-08-28"), ref, AGE_TIER_DEFAULTS)
    ).toBe("CHILD");
  });
  ...
  it("returns YOUTH for age 13 to 17 (inclusive)", () => {
    ...
    expect(
      computeAgeTierWithSettings(new Date("2013-04-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("YOUTH");
    // Born 2013-01-01: age 13 on April 1 2026 -> YOUTH
    expect(
      computeAgeTierWithSettings(new Date("2013-01-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("YOUTH");
    // Born 2009-01-01: age 17 on April 1 2026 -> YOUTH
    expect(
      computeAgeTierWithSettings(new Date("2009-01-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("YOUTH");
    // Born 2008-04-02: age 17 on April 1 2026 (hasn't turned 18 yet) -> YOUTH
    expect(
      computeAgeTierWithSettings(new Date("2008-04-02"), ref, AGE_TIER_DEFAULTS)
    ).toBe("YOUTH");
  });
  ...
  it("returns ADULT for age 18 and over", () => {
    ...
    expect(
      computeAgeTierWithSettings(new Date("2008-04-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("ADULT");
    // Born 2008-01-01: age 18 -> ADULT
    expect(
      computeAgeTierWithSettings(new Date("2008-01-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("ADULT");
    // Born 2000-01-01: age 26 -> ADULT
    expect(
      computeAgeTierWithSettings(new Date("2000-01-01"), ref, AGE_TIER_DEFAULTS)
    ).toBe("ADULT");
    // Born 1980-06-15: age 45 -> ADULT
    expect(
      computeAgeTierWithSettings(new Date("1980-06-15"), ref, AGE_TIER_DEFAULTS)
    ).toBe("ADULT");
  });
  ...
  it("Malia example: DOB 2014-08-28, season 2026 -> CHILD (age 11 on April 1 2026, CHILD goes to 12)", () => {
    ...
    expect(
      computeAgeTierWithSettings(malia, seasonStart, AGE_TIER_DEFAULTS)
    ).toBe("CHILD");
