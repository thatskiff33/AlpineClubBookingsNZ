...
// This file used to pin the environment zone to `Atlantic/Azores` with a config
// mock. #3567 deleted that environment constant and nothing reads the
// environment's zone any more, so the pin is gone; the premise below measures
// the old projection through `Atlantic/Azores` directly.
const SIGN_CHANGING_ZONE = "Atlantic/Azores";
...
describe("#3107 premise: this zone's projection is NOT a uniform shift", () => {
  it("pins both day-deltas across the 2026-03-29 transition", () => {
    ...
    expect(formatDateOnlyForTimeZone(day("2026-03-28"), SIGN_CHANGING_ZONE)).toBe("2026-03-27");
    expect(formatDateOnlyForTimeZone(day("2026-03-29"), SIGN_CHANGING_ZONE)).toBe("2026-03-28");
    // Summer time, UTC+0: the same day. Two stored days collapse onto one key
    // above, and 29 March is named by none of them.
    expect(formatDateOnlyForTimeZone(day("2026-03-30"), SIGN_CHANGING_ZONE)).toBe("2026-03-30");
    ...
    const deltas = new Set(
      NIGHTS.map(
        (value) =>
          (Date.parse(`${formatDateOnlyForTimeZone(day(value), SIGN_CHANGING_ZONE)}T00:00:00.000Z`) -
