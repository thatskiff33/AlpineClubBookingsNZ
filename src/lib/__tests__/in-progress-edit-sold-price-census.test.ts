...
const LENIENT_LOCK_CALL_SITES = [
  ...
  },
  {
    file: "src/lib/booking-modify-plan.ts",
    calls: 2,
    what: "prepareGuestPlan (the apply path's ordinary pricing pass) and `editedGuestPricingLocks` - the one wrapper (#3531) through which the apply path's proposed-remaining-guest pass AND the modify-quote preview's ordinary pricing pass reach this reader, so the two cannot disagree on which guest keeps its locks or has them cleared. The preview's in-progress branch does not use this reader at all - it goes through the planner, which is strict; since #3166 the ordinary branch is judged by preCheckInEditEvidence AFTER the pass, so the preview parks whenever the save would. The declaration itself is excluded by the pattern's lookbehind. Since #3166 the amounts this reader produces for an unreadable strand are computed and then DISCARDED: calculateModifiedPricing judges every existing strand against the night sets the pass produced and parks the edit, writing the stored price where there is one and NULL where there is not",
  },
] as const;
