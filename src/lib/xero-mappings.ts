...
// #2543's member / non-member narration lives in `rate-membership-label.ts`
// (pure, no database import) so the booking's history and audit text can say
// the same words as the invoice line (#3530, `INV-SSOT`); re-exported here so
// the Xero builders keep their one import.
export { describeGuestRateMembershipLabel } from "@/lib/rate-membership-label";
