 ...
 *    `settled + reserved` over the budget at a fixed rate (an admin rate change
 *    between one roundtrip's reserve and settle, #3354, can overshoot by that one
 *    roundtrip scaled by the change). A denied reserve runs NO side effect.
...
/**
 * `AI_PRICE_TABLE_CURRENCY` (NZD, #3354) integer cents per MILLION tokens, per
 * model. Derived from Anthropic's USD list prices multiplied by a deliberately
 * conservative FX of 1.8 NZD/USD (same as `ai-assistant-usage.ts`), so the estimate over-counts the true
