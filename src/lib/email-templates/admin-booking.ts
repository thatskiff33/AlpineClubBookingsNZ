  ...
  return layout(`
    ...
    ${infoTable([
      ...
      { label: "Reason", value: escapeHtml(data.reason) },
      {
        ...
        value: data.nights.map((night) => emailClubDate(night)).join(", "),
      },
    ])}
...
export function adminPendingDeadlineTemplate(bookings: Array<{
  ...
  const tableRowsHtml = bookings
    ...
    <tr>
      ...
      <td style="padding: 8px 12px; font-size: 14px; border-bottom: 1px solid ${p.mist}; color: ${p.deep};">${emailClubDate(b.checkIn)} – ${emailClubDate(b.checkOut)}</td>
...
export function adminCapacityWarningTemplate(days: Array<{
  ...
  const tableRowsHtml = days
    .map((d) => {
      ...
      return `
    <tr>
      <td style="padding: 8px 12px; font-size: 14px; border-bottom: 1px solid ${p.mist}; color: ${p.deep};">${emailClubDate(d.date)}</td>
...
export function adminBookingChangeRequestTemplate(data: {
  ...
  return layout(`
    ...
    ${infoTable([
      ...
      { label: "Booking", value: escapeHtml(data.bookingId) },
      { label: "Current check-in", value: emailClubDate(data.checkIn) },
      { label: "Current check-out", value: emailClubDate(data.checkOut) },
