...
export function refundRequestApprovedTemplate(data: {
  ...
  return refundRequestOutcomeLayout({
    ...
    outcomeSentence:
      "Your refund appeal for your booking (" + emailClubDate(data.checkIn) + " - " + emailClubDate(data.checkOut) + ") has been approved. A refund of " + formatCents(data.amountCents ?? 0) + " will be processed to your original payment method.",
    outcomeTone: "success",
...
export function refundRequestDeclinedTemplate(data: {
  ...
  return refundRequestOutcomeLayout({
    ...
    outcomeSentence:
      "Your refund appeal for your booking (" + emailClubDate(data.checkIn) + " - " + emailClubDate(data.checkOut) + ") was not approved at this time.",
    outcomeTone: "warning",
    adminNotes: data.adminNotes,
  });
}
