...
describe("formatStayDate keeps the stored lodge night west of Greenwich (INV-DATE-010)", () => {
  it("reads the encoded day, so a club behind Greenwich sees the night it booked", () => {
    expect(formatStayDate(SERIALISED_STAY)).toBe(EXPECTED);
    expect(formatStayDate(new Date(SERIALISED_STAY))).toBe(EXPECTED);
  });
  ...
  it("is the route that differs from reading the same instant through a zone", () => {
    ...
    expect(formatClubInstantDate(instant, VANCOUVER)).toBe("15 Aug 2026");
    expect(formatClubInstantDate(instant, AUCKLAND)).toBe(EXPECTED);
    expect(formatStayDate(instant)).toBe(EXPECTED);
  });

  it("does not depend on the host's zone either", () => {
    for (const hostZone of ["America/Vancouver", "Pacific/Honolulu", "Pacific/Auckland"]) {
      expect(withTimeZone(hostZone, () => formatStayDate(SERIALISED_STAY))).toBe(EXPECTED);
      expect(withTimeZone(hostZone, () => formatStayDate(new Date(SERIALISED_STAY)))).toBe(
        EXPECTED,
      );
    }
  });
  ...
  it("accepts the bare yyyy-MM-dd spelling a route may already have encoded", () => {
    expect(formatStayDate("2026-08-16")).toBe(EXPECTED);
  });

  it("throws for a value that is not a calendar day, rather than rendering a guess", () => {
    expect(() => formatStayDate("not-a-date")).toThrow();
    expect(() => formatStayDate("2026-02-30")).toThrow();
  });
});

describe("formatStayDateOrNull is the client-render form", () => {
  it("formats the same value the same way", () => {
    expect(formatStayDateOrNull(SERIALISED_STAY)).toBe(EXPECTED);
    expect(formatStayDateOrNull("2026-08-16")).toBe(EXPECTED);
  });

  it("answers null for an absent or malformed value so the caller picks the fallback", () => {
    expect(formatStayDateOrNull(null)).toBeNull();
    expect(formatStayDateOrNull(undefined)).toBeNull();
    expect(formatStayDateOrNull("")).toBeNull();
    expect(formatStayDateOrNull("not-a-date")).toBeNull();
    expect(formatStayDateOrNull("2026-02-30")).toBeNull();
    expect(formatStayDateOrNull("rubbish") ?? "—").toBe("—");
  });
});
