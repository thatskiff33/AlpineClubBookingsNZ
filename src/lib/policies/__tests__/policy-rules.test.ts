...
describe("policy subscription rules", () => {
  it("uses per-tier subscription settings and defaults missing tiers to required", () => {
    expect(
      requiresPaidSubscriptionForAgeTier("INFANT", AGE_TIER_DEFAULTS)
    ).toBe(false);
    expect(requiresPaidSubscriptionForAgeTier("CHILD", AGE_TIER_DEFAULTS)).toBe(
      false
    );
    expect(requiresPaidSubscriptionForAgeTier("YOUTH", AGE_TIER_DEFAULTS)).toBe(
      true
    );
    expect(
      requiresPaidSubscriptionForAgeTier(undefined, AGE_TIER_DEFAULTS)
    ).toBe(true);
  });
});
