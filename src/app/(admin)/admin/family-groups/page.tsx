...
export default function FamilyGroupsPage() {
  ...
  const {
    ...
    addMember,
    removeMember,
  } = useFamilyGroupMemberPicker();

  const { initializeRequestState, getRequestReviewCardProps } =
    useFamilyGroupRequestReview({
      onRefresh: () => fetchData(),
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (requestsRes.ok) {
        ...
        initializeRequestState(fetchedRequests);
      }
  ...
  const openEditForm = useCallback((group: FamilyGroupSummary) => {
    ...
    setError("");
    setShowForm(true);
  }, [setSelectedMembers, setMemberSearch, setSearchResults]);
          ...
          {loading ? (
            ...
            requests.map((request) => (
              <FamilyGroupRequestReviewCard
                key={request.id}
                {...getRequestReviewCardProps(request)}
                showSearchGuidance
              />
            ))
          )}
