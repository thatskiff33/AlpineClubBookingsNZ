...
/**
 * #2930 — the booking-EDIT quote is a member-facing capacity refusal too.
 *
 * The settled owner contract says a hold-only refusal must be indistinguishable
 * from genuine fullness "in copy, in the night list AND in the payload", and
 * `INV-CAP-021` now asserts exactly that. The create path was collapsed onto
 * `getCapacityFullNights`; this route was not. The member-facing price summary
 * card built its own list from `nightDetails.filter(n => n.availableBeds < 0)`
 * and printed "N bed(s) short" per row.
 *
 * THAT LEAKED TWICE OVER. A whole-lodge-held night is pinned to exactly 0
 * available beds and never goes negative (the pin is what keeps it out of the
 * admin confirmable set, ADR-001 decision 5), so it fell out of that filter: a
 * hold-only refusal drew an EMPTY list under the same "Not enough beds
 * available" sentence that an ordinary full lodge drew an itemised one under.
 * That is a clean detector in the network response before a pixel is drawn. The
 * per-night shortfall then told the two apart a second way, by arithmetic,
 * because a held night has no shortfall to state.
 *
 * Every assertion below fails against the pre-fix route: `capacityFullNights`
 * did not exist, and `nightDetails` reached a member with the bed numbers on it.
 *
 * The harness is the in-progress minimum-stay suite's, which is the smallest one
 * in this directory that gets a MEMBER through to a 200 quote carrying a
 * capacity refusal. What differs is below the fold.
 */
...
vi.mock("@/lib/booking-modify", () => ({
  ...
  // #3531: the route reaches the reader through this wrapper; the null it
    ...
    guests: [
      {
        ...
        priceCents: 30000,
        nights: [
          { stayDate: D("2026-08-14"), priceCents: 7500, priceSource: "SOLD" },
          { stayDate: D("2026-08-15"), priceCents: 7500, priceSource: "SOLD" },
          { stayDate: D("2026-08-16"), priceCents: 7500, priceSource: "SOLD" },
          { stayDate: D("2026-08-17"), priceCents: 7500, priceSource: "SOLD" },
        ],
      },
    ],
  };
...
describe("POST /api/bookings/[id]/modify-quote — hold privacy in the refusal (#2930)", () => {
  /**
   * The engine's answer for a night the lodge is exclusively held on: a full
   * lodge, ZERO free beds, never a negative number, and the flag no
   * member-facing payload may carry (`INV-CAP-021`, `INV-CAP-038`).
   */
  ...
  /** The engine's answer for a night that simply will not fit the party. */
  const OVER_CAPACITY = (date: string, short: number) => ({
    date: D(date),
    occupiedBeds: 29 + short,
    availableBeds: -short,
    wholeLodgeHeld: false,
  });

  async function memberRefusal(nightDetails: unknown[]) {
    asMember();
    h.checkCapacityForGuestRanges.mockResolvedValue({
      available: false,
      minAvailable: 0,
  ...
  it("names a whole-lodge-held night, whose availableBeds is 0 rather than negative", async () => {
    const body = await memberRefusal([HELD("2026-08-19")]);

    expect(body.capacityFullNights).toEqual(["2026-08-19"]);
  });

  it("gives a hold-only refusal and a genuine-fullness refusal the SAME night list", async () => {
    // Stated as an equality rather than as two plausible-looking expectations,
    // because what matters is that a reader cannot tell them apart.
    const heldRefusal = await memberRefusal([HELD("2026-08-18"), HELD("2026-08-19")]);
    const fullRefusal = await memberRefusal([
      OVER_CAPACITY("2026-08-18", 2),
      OVER_CAPACITY("2026-08-19", 5),
    ]);

    expect(heldRefusal.capacityFullNights).toEqual(fullRefusal.capacityFullNights);
    expect(heldRefusal.capacityFullNights).toEqual(["2026-08-18", "2026-08-19"]);
  });

  it("sends a member no per-night bed numbers at all", async () => {
    // The shortfall is what tells the two refusals apart once the lists match:
    // a held night has none to state, so the member's payload carries dates and
    // nothing else.
    const body = await memberRefusal([
      HELD("2026-08-18"),
      OVER_CAPACITY("2026-08-19", 7),
    ]);

    expect(body.nightDetails).toBeUndefined();
    for (const night of body.capacityFullNights) {
      expect(night).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    }
  });

  it("never projects the hold under any name", async () => {
    const serialised = JSON.stringify(await memberRefusal([HELD("2026-08-19")]));

    expect(serialised).not.toContain("wholeLodgeHeld");
    expect(serialised.toLowerCase()).not.toContain("exclusive");
  });

  it("still gives an ADMIN OVERRIDE the confirmable nights, with held ones excluded", async () => {
    // The other end of the same rule: a held night is never confirmable, so it
    // must not enter the set an admin is offered a confirm against (decision 5)
    // — while still appearing in the refusal's own night list. The bed numbers
    // live here, on a branch `adminOverride` gates, and nowhere else.
    h.checkCapacityForGuestRanges.mockResolvedValue({
      available: false,
      minAvailable: 0,
      nightDetails: [HELD("2026-08-18"), OVER_CAPACITY("2026-08-19", 3)],
    });

    const res = await POST(
      req({ checkOut: "2026-08-20", adminOverride: true, pricingMode: "recalculate" }),
      { params },
    );
    ...
    expect(body.nightDetails).toEqual([{ date: "2026-08-19", availableBeds: -3 }]);
    expect(body.capacityFullNights).toEqual(["2026-08-18", "2026-08-19"]);
  });
});
